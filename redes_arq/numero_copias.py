...
cobriu = True
ncopias = 1
#enquanto dá certo, tenta colocar mais uma cópia
while (cobriu):
    print("Numero de cópias : "+str(ncopias)+"\n")
    ...
    for ciclos in range(0,MAX_CICLOS) :
        ...
        for line in infile.readlines() :
            ...
            #criar cópias da rede
            copia = []
            for c in ncopias :
                copia.append([[],{}])
                for pe in PE_rede:
                    copia[c][0].append(pe)
                    copia[c][1][pe] = qtd_rede[pe]
            ...
            for ciclo in range(0,ciclos+1) :
                ...
                #avaliar a arquitetura para cada cópia
                for c in ncopias:
                    PE_rede = copia[c][0]
                    qtd_rede = copia[c][1]
                    for pe in PE_rede :
